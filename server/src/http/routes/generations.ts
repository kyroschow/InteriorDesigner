...
import { type GenerationRecord, type GenerationScope, isDuplicateKeyError } from '../../db/store.ts'
...
export function generationRoutes(app: FastifyInstance, ctx: AppContext) {
  ...
  app.post<{ Params: Params }>('/projects/:projectId/generations', async (request, reply) => {
    ...
    const createOrReuse = () =>
      store.withTransaction(async (session) => {
        ...
        // Serialize generation requests per project so duplicates cannot race past the checks below.
        await store.lockProject(projectId, session)
        const timestamp = new Date().toISOString()
        if (rawKey) {
          const existing = await store.getIdempotency(projectId, rawKey, session)
          if (existing) {
            ...
            return (await store.getGeneration(projectId, existing.generationId, session))!
          }
        }
        ...
        let record = await store.findActiveGeneration(projectId, scope, project.revision, project.configuration.revision, session)
        if (!record) {
          const layout = project.activeLayoutId ? await store.getLayout(projectId, project.activeLayoutId, session) : null
          record = {
            id: newId('gen'),
            projectId,
            status: 'queued',
          ...
          await store.insertGeneration(record, session)
        }
        if (rawKey) await store.insertIdempotency(projectId, rawKey, requestHash, record.id, timestamp, session)
        return record
      })

    // A concurrent request may insert the same Idempotency-Key first; retrying then finds and returns it.
    const generation = await createOrReuse().catch((err) => {
      if (isDuplicateKeyError(err)) return createOrReuse()
      throw err
    })
  ...
  app.get<{ Params: Params & { generationId: string } }>('/projects/:projectId/generations/:generationId', async (request) => {
    await projects.require(request.params.projectId)
    const g = await store.getGeneration(request.params.projectId, request.params.generationId)
  ...
  app.get<{ Params: Params & { layoutId: string } }>('/projects/:projectId/layouts/:layoutId', async (request) => {
    await projects.require(request.params.projectId)
    const layout = await store.getLayout(request.params.projectId, request.params.layoutId)
